...
def test_lead_crud_and_search(client, session):
    ...
    assert client.get("/api/v1/leads").json()["total"] == 1
...
def test_leads_isolated_between_tenants(client, session):
    ...
    assert client.get("/api/v1/leads").json()["total"] == 1
    assert client_b.get("/api/v1/leads").json()["items"] == []  # B sees none of A's leads

    lead_id = client.get("/api/v1/leads").json()["items"][0]["id"]
